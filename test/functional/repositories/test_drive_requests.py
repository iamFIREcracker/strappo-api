...
class TestDriveRequestsRepository(unittest.TestCase):
    ...
    def test_get_all_active_by_passenger_does_not_return_requests_linked_to_deleted_users(self):
        # Given
        self.session.add(User(id='uid', name='Name', avatar='Avatar', deleted=True))
        ...
        # When
        requests = DriveRequestsRepository.get_all_active_by_passenger('pid')

        # Then
        self.assertEquals([], requests)

    def test_get_all_active_by_passenger_does_not_return_drive_requests_not_active(self):
        # Given
        self.session.add(User(id='uid', name='Name', avatar='Avatar'))
        self.session.add(Driver(id='pid', user_id='uid'))
        self.session.add(DriveRequest(id='rrid', driver_id='did',
                                     passenger_id='pid', accepted=True,
                                     active=False))
        self.session.commit()
    ...
    def test_get_all_active_by_passenger(self):
        # Given
        self.session.add(User(id='uid', name='Name', avatar='Avatar'))
        self.session.add(Driver(id='did', user_id='uid'))
        self.session.add(DriveRequest(id='rrid', driver_id='did',
